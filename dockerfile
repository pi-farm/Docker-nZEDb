...
RUN apt-get update && apt-get upgrade -y && apt-get autoremove -y && apt-get install --no-install-recommends nano wget mariadb-server mariadb-client curl unzip git php7.2 php7.2-fpm php7.2-mysql php7.2-common php7.2-gd php7.2-json php7.2-cli php7.2-curl libapache2-mod-php7.2 php-imagick php-pear php7.2-dev php7.2-mbstring php7.2-xml apparmor-utils time p7zip-full mediainfo lame ffmpeg zip unrar-free libevent-dev build-essential git autotools-dev automake pkg-config ncurses-dev python python-setuptools python-dev build-essential python-pip ca-certificates -y
...
RUN cd /var/www/nZEDb/ && su -c "composer install" -s /bin/bash www-data
